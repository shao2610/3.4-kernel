...
/* [LGE_UPDATE_S] : For Fuel Gauge - 20120412*/
...
extern int nv_soc_val;
int msm_rw_fuel(uint32_t event, uint8_t is_write, char *buf, uint32_t len)
{
	struct oem_rapi_client_streaming_func_arg arg;
	struct oem_rapi_client_streaming_func_ret ret;
	int rc= -1;
...
EXPORT_SYMBOL(msm_rw_fuel);
/*  [LGE_UPDATE_E] : For Fuel Gauge - 20120412 */
...
EXPORT_SYMBOL(msm_rw_felica);

#define RW_ANNOYING_FLC_FAIL -1
#define RW_ANNOYING_FLC_NOT_ACTIVE 0
#define ANNOYING_FLC_NOT_ACTIVE_MARK (0xDEAD)
...
EXPORT_SYMBOL(msm_rw_annoying_flc);
#endif
